...
def pf_tracker(video, text, frames_to_save=[], play_video=True, num_particles=100, dimensions=2,
    ...
    search_space = np.array(frame.shape)
    ...
    cv2.imwrite('results/msl/firstframe_imagepatch.png', frame[miny:maxy, minx:maxx])

    pftracker = MSL_PF_Tracker(model, search_space, num_particles, dimensions, control, sim_std, alpha)
    ...
    while capture.isOpened():
        ...
        if frame_count in frames_to_save:
            cv2.imwrite('results/msl/frame-'+str(frame_count)+'.png', frame)
            saved_frames_count

if __name__ == '__main__':
    print("Mean Shift Line Particle Filter Tracking Started")
    ...
    print("Mean Shift Line Particle Filter Tracking End")

    print("Mean Shift Line Particle Filter Tracking with appearance model update Started")
    pf_tracker(videos[2], text[3], [28,84,144], play_video=True, num_particles=700, dimensions=2,
                          control=10, sim_std=20, alpha=0) #Function to start Particle Filter Tracking
    print("Mean Shift Line Particle Filter Tracking with appearance model update End")
